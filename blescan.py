...
'30da147e6d6f4b3693e6e053f1b7f24e']
...
def get_dup(myFullList, key, value):
    ...
    for i in range(0, loop_count):
		...
		i =0
		    ...
		    mac = packed_bdaddr_to_string(pkt[report_pkt_offset + 3:report_pkt_offset + 9])
                    txp = "%lf" %struct.unpack("b", pkt[report_pkt_offset -2])
                    rssi = "%i" %struct.unpack("b", pkt[report_pkt_offset -1])
                    dist = calc_dist(int(rssi), float(txp))

                    Adstring =  { 'uuid' : uuid, 'txp' : txp, 'rssi' : rssi, 'dist' : dist}
